...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const { kind, targetUserId } = req.body as z.infer<typeof orderSchema>;
    ...
    if (kind === "premium") {
      ...
      target = user.id;
      baseUzs = settings.premiumPriceUzs ?? DEFAULT_PREMIUM_UZS;
    } else {
      // Boost or gift.
    ...
    const order = await createCardOrder(orderKind, user.id, target, baseUzs);
    if (!order) {
...
router.post("/cardxabar", async (req: Request, res: Response) => {
  ...
  try {
    if (order.kind === CardOrderKind.premium) {
      await grantPremium(order.targetUserId);
    } else {
