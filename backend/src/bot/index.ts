...
bot.on("message:successful_payment", async (ctx) => {
  ...
  // ----- Premium: premium:<userId> or premium:<userId>:<days> -----
  const premiumMatch = /^premium:([^:]+)(?::(\d+))?$/.exec(payload);
  if (premiumMatch) {
    const userId = premiumMatch[1];
    const days = premiumMatch[2] ? parseInt(premiumMatch[2], 10) : undefined;
    try {
      const until = await grantPremium(userId, days);
      if (until) {
