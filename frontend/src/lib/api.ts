...
export const api = {
  ...
  getLikes: () =>
    request<{ likes: PublicProfile[]; newCount: number; unlockedCount: number }>(
      "/api/likes",
    ),

  createPremiumInvoice: (days?: number) =>
  ...
  createPremiumCardOrder: (days?: number) =>
    request<CardOrder>("/api/payments/card/order", {
      method: "POST",
      body: JSON.stringify(days ? { kind: "premium", days } : { kind: "premium" }),
    }),
